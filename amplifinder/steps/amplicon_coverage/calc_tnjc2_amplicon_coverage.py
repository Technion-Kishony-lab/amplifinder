...
class CalcTnJc2AmpliconCoverageStep(RecordTypedDfStep[CoveredTnJc2]):
    ...
    def _calc_candidate_coverage(
        ...
        if self.has_ancestor and anc_scaf_cov is not None and anc_scaf_median is not None:
            ...
            anc_cp = anc_region_cov.astype(
                float) / anc_scaf_median if anc_scaf_median > 0 else anc_region_cov.astype(float)
            ...
            # Calculate raw copy number using scaffold-specific median
            ncp = iso_region_cov.astype(
                float) / iso_scaf_median if iso_scaf_median > 0 else iso_region_cov.astype(float)
        ...
        iso_amplicon_cov_stats = calc_coverage_stats(iso_region_cov)
        scaf_coverage = calc_coverage_stats(iso_scaf_cov)
        anc_amplicon_cov_stats = (
            calc_coverage_stats(anc_region_cov) if anc_region_cov is not None else None
        )
        anc_scaf_cov_stats = calc_coverage_stats(anc_scaf_cov) if self.has_ancestor else None
