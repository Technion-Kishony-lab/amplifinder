...
class Schema:
    ...
    @classmethod
    def from_csv(cls, path: Path, allow_extra: bool = True, *, fields_col: str = "fields", types_col: str = "types", optional_col: str = "optional") -> Schema:
        """Load schema from CSV file with columns: fields, types, optional."""
        df = pd.read_csv(path, skipinitialspace=True)
        return Schema(
            columns=tuple(Column(row[fields_col], _TYPE_MAP[row[types_col]], bool(row[optional_col])) for _, row in df.iterrows()),
