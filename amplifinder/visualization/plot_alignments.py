...
def _down_sample_alignments(alignments: List[Tuple[int, int, ReadType]], jc_cov: JunctionReadCounts, max_reads_per_plot: int, arm_len: int, min_overlap_len: int, read_len: int) -> tuple[List[Tuple[int, int, ReadType]], JunctionReadCounts]:
...
def plot_junctions_coverage(
    ...
    for jt in JunctionType:
        ...
        jc_cov = jc_covs[jt]
        downsampled, scales_iso = _down_sample_alignments(alignments, jc_cov, max_reads_per_plot, arm_len, min_overlap_len, read_len)
        ...
        if alignment_data_anc:
            ...
            downsampled_anc, scales_anc = _down_sample_alignments(alignments_anc, jc_cov_anc, max_reads_per_plot, arm_len, min_overlap_len, read_len)
