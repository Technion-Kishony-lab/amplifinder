...
if bam_path.exists():
    ...
    combined_alignments = {
        k: v for k, v in combined.items() if isinstance(
            v, CombinedSingleAlignment) and len(v.bam_indices) > 1
    }
    ...
    for i, (key, alignment) in enumerate(list(combined_alignments.items())[:5]):
        ...
        msg = (f"   Combined span: {alignment.start} - {alignment.end} "
               f"({alignment.end - alignment.start} bp)")
        print(msg)
